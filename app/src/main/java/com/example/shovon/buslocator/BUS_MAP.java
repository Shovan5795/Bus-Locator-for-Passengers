...
public class BUS_MAP extends FragmentActivity implements
...
//            Toast.makeText(MapsActivity.this, "null", Toast.LENGTH_SHORT).show();
    ...
    private void handleNewLocation(Location location) {
        ...
        final String lng=String.valueOf(currentLongitude);

        Toast.makeText(getApplicationContext(), "Longitude"+lng, Toast.LENGTH_SHORT).show();

        firebase=new Firebase(Config.FIREBASE_URL);

        firebase.child("Bus").child(id).child("lat").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                lt=(String)dataSnapshot.getValue();
                firebase.child("Bus").child(id).child("long").addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ln=(String)dataSnapshot.getValue();
                        Double d=distance(Double.parseDouble(lat), Double.parseDouble(lng),Double.parseDouble(lt), Double.parseDouble(ln));

                        DecimalFormat df = new DecimalFormat("#.##");
                        d = Double.valueOf(df.format(d));
                        dist.setText(Double.toString(d));
